...
| [duarouter](duarouter.md)                                    | 計算路網中最快的路徑、並可匯入不同的需求。運行動態交通量指派。 |
...
另外，SUMO 還使用更快的資料結構，每一個元件都被打造成符合使用目的，而非複雜的形式。不過，這方法讓 SUMO 在使用上可能比其他模擬軟體的使用體驗還要不舒服一些，這個我們是知道的。但是為了發展其他功能，我們目前還不打算重新設計一個整合所有功能的 SUMO 版本。
